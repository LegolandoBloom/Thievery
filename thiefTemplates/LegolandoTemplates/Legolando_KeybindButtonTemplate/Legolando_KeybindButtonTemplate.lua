...
Legolando_KeybindButtonMixin_Thievery = {}

Legolando_KeybindButtonMixin_Thievery.Modifiers = {
...
function Legolando_KeybindButtonMixin_Thievery:CallOnBindFunction()
    if self.onBindFunction then
        self.onBindFunction()
    end
end

function Legolando_KeybindButtonMixin_Thievery:checkTableAndReference()
...
function Legolando_KeybindButtonMixin_Thievery:UpdateSavedVariables(base, modifier)
    local teeburu = self.savedVarTable
    ...
    if modifier then
        ...
        EventRegistry:TriggerEvent("Lego-KeyBound-Thievery-" .. self:GetParentKey(), base, modifier)
    elseif base then
        ...
        EventRegistry:TriggerEvent("Lego-KeyBound-Thievery-" .. self:GetParentKey(), base)
    else
        teeburu[self.keybindRef] = nil
        if self.modifierRef then teeburu[self.modifierRef] = nil end
        if self.baseRef then teeburu[self.baseRef] = nil end
        EventRegistry:TriggerEvent("Lego-KeyUnbound-Thievery-" .. self:GetParentKey())
    end
end

function Legolando_KeybindButtonMixin_Thievery.OnClick(self, button, down)
...
function Legolando_KeybindButtonMixin_Thievery.OnUp(self, key)
...
function Legolando_KeybindButtonMixin_Thievery.Mouse(self, button)
...
function Legolando_KeybindButtonMixin_Thievery.GamePad(self, button)
    local teeburu = self.savedVarTable
    local keybindRef = self.keybindRef
    self:SetScript("OnKeyUp", nil)
    self:SetScript("OnKeyDown", nil)
...
function Legolando_KeybindButtonMixin_Thievery.MouseWheel(self, delta)
    local scroll
    if delta == 1 then
        scroll = "MOUSEWHEELUP"
    elseif delta == -1 then
...
function Legolando_KeybindButtonMixin_Thievery.Modified(self, key)
    local teeburu = self.savedVarTable
    local keybindRef = self.keybindRef
    if key == "ENTER" then
...
function Legolando_KeybindButtonMixin_Thievery:StopWatching()
    local teeburu = self.savedVarTable
    local keybindRef = self.keybindRef
...
function Legolando_KeybindButtonMixin_Thievery:Unbind(self)
    local teeburu = self.savedVarTable
    local keybindRef = self.keybindRef
